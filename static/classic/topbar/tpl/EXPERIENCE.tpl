...
<script>
  ...
  function init() {
    ...
    Data.Experience.sort(function(a, b) {
  ...
  function refresh() {
    ...
    $.each(Data.Experience, function(k, v) {
      if (v != null && v.section_type == "EXPERIENCE") {
        temp += '<a style="border-radius:10px;box-shadow:0 1px 1px rgba(0,0,0,0.15),-1px 0 0 rgba(0,0,0,0.03),1px 0 0 rgba(0,0,0,0.03),0 1px 0 rgba(0,0,0,0.12);    background: rgba(245, 245, 245, 0.2);" class="list-group-item margin-15" href="javascript:void(0)" id="' + k + '"  onMouseOver="toolOptionShow(' + k + ')" onMouseOut="toolOptionHide(' + k + ')">' +
          ...
          '<img src="/static/classic/topbar/assets/images/chars/'+ v.section_title2.charAt(0).toUpperCase() +'.png" onerror = "$(this).remove();" alt="...">' +
          '</div>' +
          '</div>' +
    ...
    temp += '<button type="button" class="btn btn-sm btn-outline btn-default width-250 font-weight-400 center-block addnewBtn margin-top-20 margin-bottom-50"><i class="icon wb-plus"></i>&nbsp;Add Experience</button>';
    ...
    $(".allowSort").sortable({
      ...
      update: function(event, ui) {
        ...
        $.each(sectionOrder, function(k, v) {
          if (v != "") {
            Data.Experience[v].order_prefs = orderpref;
            orderpref++;
          }
        });
    ...
    $('div.media-body').on('click', function(e) {
      ...
      if ($('.editform').is(':visible')) {
        ...
        $(".eimg").attr('src',"/static/classic/topbar/assets/images/chars/"+Data.Experience[$(this).data('1')].section_title2.charAt(0).toUpperCase()+".png");
        $(".esec1").val(Data.Experience[$(this).data('1')].section_title1);
        $(".esec2").val(Data.Experience[$(this).data('1')].section_title2);
        $(".esec3").val(Data.Experience[$(this).data('1')].section_title3);
        var res = Data.Experience[$(this).data('1')].start_date.split(' ');


        $(".estmn").val($.trim(res[0]));
        $(".estyr").val($.trim(res[1]));
        res = Data.Experience[$(this).data('1')].end_date.split(' ');
        ...
        $(".edeon").val(Data.Experience[$(this).data('1')].more_info1);
        $(".edeon").summernote({});
        ...
        $('.esec1').on("input", function() {
          $('.EXP-' + $('.eid').html() + '.E1').html($('.esec1').val().toUpperCase());
        });
        ...
        $('.esec2, .esec3, .estyr, .estmn, .eenmn, .eenyr, .eprnt').on("input", function() {
          ...
          $('.EXP-' + $('.eid').html() + '.E2').html(subtitle);
        });
        
        $('.editform .note-editor').on("input", function() {
          $('.EXP-' + $('.eid').html() + '.E3').html($(".edeon").code());
        });
  ...
  function updateRecord(key) {
    ...
    if (Data.Experience[key] != null && Data.Experience[key].section_type == "EXPERIENCE") {
      Data.Experience[key].section_title1 = $(".esec1").val();
      Data.Experience[key].section_title2 = $(".esec2").val();
      Data.Experience[key].section_title3 = $(".esec3").val();

      Data.Experience[key].start_date = $.trim($('.estmn').val())+' '+$.trim($('.estyr').val());


      if($(".eprnt").prop('checked') == true){
        Data.Experience[key].end_date = 'Present';
      }else{
        Data.Experience[key].end_date = $.trim($('.eenmn').val())+' '+$.trim($('.eenyr').val());
        if($.trim(Data.Experience[key].end_date) == '-' || $.trim(Data.Experience[key].end_date) == ''){
          Data.Experience[key].end_date = '';
        }
      }
      if($(".edeon").code().replace(/<\/?[^>]+(>|$)/g, "") == ""){
        Data.Experience[key].more_info1 = "";
      }else{
        Data.Experience[key].more_info1 = $(".edeon").code();
      }      
      localStorage.setItem("data", JSON.stringify(Data));
      localStorage.setItem("modified_dt", Date.now());
      buildresume();
  ...
  function changeVisibility(key, status) {
    ...
    if (status == 1) {
      Data.Experience[key].is_active = 0;
    }
    else {
      Data.Experience[key].is_active = 1;
    }
    localStorage.setItem("data", JSON.stringify(Data));
    localStorage.setItem("modified_dt", Date.now());
    buildresume();
...
<div class="nav_tab_container">
  ...
  <div style="float:left;width:64%;line-height: 45px;">
    ...
    <ul class="site-sidebar-nav nav nav-tabs nav-justified nav-tabs-line scrollable scrollable-horizontal" role="tablist" >
      ...
      <li style="cursor:pointer;font-weight:400;" role="presentation">
        <a class="active_menu" data-toggle="slidePanel" aria-hidden="true" data-url="../../static/classic/topbar/tpl/EXPERIENCE.tpl"><span>EXPERIENCE</span>
        </a>
      </li>
      <li style="cursor:pointer;font-weight:400;" role="Presentation">
        ...
        <form class='newForm margin-bottom-50 padding-10' style="margin-right: -10px; margin-left: -10px;">
          <div class="newForm-header" style="margin: -10px;margin-bottom:15px;">
            <span class="new-heading pull-right" style=" margin-top: -10px; line-height: 25px; text-align: right;">
              <i class="fa fa-close" style="font-color:red;cursor:pointer;"></i><br>ADD EXPERIENCE
            </span>
            <div class="avatar avatar-sm" style="width:35px;">
