...
<script>
  ...
  function init() {
    ...
    Data.Certification.sort(function(a, b) {
  ...
  function refresh() {
    ...
    $.each(Data.Certification, function(k, v) {
      if (v != null && v.section_type == "CERTIFICATION") {
    ...
    temp += '<button type="button" class="btn btn-sm btn-outline btn-default width-250 font-weight-400 center-block addnewBtn margin-top-20 margin-bottom-50"><i class="icon wb-plus"></i>&nbsp;Add Certification</button>';
    ...
    $(".allowSort").sortable({
      ...
      update: function(event, ui) {
        ...
        $.each(sectionOrder, function(k, v) {
          if (v != "") {
            Data.Certification[v].order_prefs = orderpref;
            orderpref++;
          }
        });
    ...
    $('div.media-body').on('click', function(e) {
      ...
      if ($('.editform').is(':visible')) {
        ...
        $(".eimg").attr('src',"/static/classic/topbar/assets/images/chars/"+Data.Certification[$(this).data('1')].section_title2.charAt(0).toUpperCase()+".png");
        $(".esec2").val(Data.Certification[$(this).data('1')].section_title1);
        $(".esec1").val(Data.Certification[$(this).data('1')].section_title2);
        $(".esec3").val(Data.Certification[$(this).data('1')].section_title3);
        var res = Data.Certification[$(this).data('1')].start_date.split(' ');
        $(".estmn").val($.trim(res[0]));
        $(".estyr").val($.trim(res[1]));
        

        $(".edeon").val(Data.Certification[$(this).data('1')].more_info1);
        $(".edeon").summernote();
        ...
        $('.esec1').on("input", function() {
          $('.CERT-' + $('.eid').html() + '.E1').html($('.esec1').val().toUpperCase());
        });
        ...
        $('.esec2, .esec3, .estyr, .estmn').on("input", function() {
          ...
          $('.CERT-' + $('.eid').html() + '.E2').html(subtitle);
        });
        
        $('.editform .note-editor').on("input", function() {
          $('.CERT-' + $('.eid').html() + '.E3').html($(".edeon").code());
        });
  ...
  function updateRecord(key) {
    ...
    if (Data.Certification[key] != null && Data.Certification[key].section_type == "CERTIFICATION") {
      Data.Certification[key].section_title1 = $(".esec2").val();
      Data.Certification[key].section_title2 = $(".esec1").val();
      Data.Certification[key].section_title3 = $(".esec3").val();

      Data.Certification[key].start_date = $.trim($('.estmn').val())+' '+$.trim($('.estyr').val());
      if($(".eprnt").prop('checked') == true){
        Data.Certification[key].end_date = 'Present';
      }else{
        Data.Certification[key].end_date = $.trim($('.eenmn').val())+' '+$.trim($('.eenyr').val());
        if($.trim(Data.Certification[key].end_date) == '-' || $.trim(Data.Certification[key].end_date) == ''){
          Data.Certification[key].end_date = '';
        }
      }
  ...
  function changeVisibility(key, status) {
    ...
    if (status == 1) {
      Data.Certification[key].is_active = 0;
    }
    else {
      Data.Certification[key].is_active = 1;
    }
    localStorage.setItem("data", JSON.stringify(Data));
    localStorage.setItem("modified_dt", Date.now());
    buildresume();
...
<div class="nav_tab_container">
  ...
  <div style="float:left;width:64%;line-height: 45px;">
    ...
    <ul class="site-sidebar-nav nav nav-tabs nav-justified nav-tabs-line scrollable scrollable-horizontal" role="tablist" >
      ...
      <li style="cursor:pointer;font-weight:400;" role="presentation">
        <a class="active_menu" data-toggle="slidePanel" aria-hidden="true" data-url="../../static/classic/topbar/tpl/CERTIFICATIONS.tpl"><span>CERTIFICATIONS</span>
        </a>
      </li>
      <li style="cursor:pointer;font-weight:400;" role="presentation" >
...
<div class="site-sidebar-tab-content tab-content">
  ...
  <div class="tab-pane fade active in" id="sidebar-editing" style="opacity:0;">
    ...
    <div class="">
      ...
      <div class="">
        <span class="slidePanel-desc">Enter details of certifications you have qualified</span>
        <br>
        <br>
        ...
        <form class='newForm margin-bottom-50 padding-10' style="margin-right: -10px; margin-left: -10px;">
          <div class="newForm-header" style="margin: -10px;margin-bottom:15px;">
            <span class="new-heading pull-right" style=" margin-top: -10px; line-height: 25px; text-align: right;">
              <i class="fa fa-close" style="font-color:red;cursor:pointer;"></i><br>ADD CERTIFICATION
            </span>
            <div class="avatar avatar-sm" style="width:35px;">
              <img src="/static/classic/topbar/assets/images/chars/new.png" alt="...">
            </div>
          </div>
          <!--<span class="new-heading">ADD CERTIFICATION</span><i class="icon wb-close pull-right cancelnewadd" style="cursor:pointer;color:#DE5F5F;margin-right: 8px;"></i>-->
          <div class="form-group form-material floating">
            <input type="text" class="form-control empty sec1" />
            <label class="floating-label">Certification Title</label>
          </div>

          <div class="form-group form-material floating">
            <input type="text" class="form-control empty sec2" />
            <label class="floating-label">Issuer Name</label>
          </div>
        ...
        <div class="row margin-bottom-50">
          ...
          <div class="col-sm-5 col-xs-5 ">
            <button type="button" class="btn btn-block btn-warning font-weight-400" style="letter-spacing:.1em;" data-toggle="slidePanel" aria-hidden="true" data-url="../../static/classic/topbar/tpl/LANGUAGES.tpl">NEXT&nbsp; <i class="fa fa-chevron-right"></i> </button>
          </div>
        </div>
