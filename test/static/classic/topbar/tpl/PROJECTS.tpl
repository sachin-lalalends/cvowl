...
<script>
  ...
  function refresh() {
    ...
    $.each(Data.Project, function(k, v) {
      if (v != null && v.section_type == "PROJECT") {
        temp += '<a style="border-radius:10px;box-shadow:0 1px 1px rgba(0,0,0,0.15),-1px 0 0 rgba(0,0,0,0.03),1px 0 0 rgba(0,0,0,0.03),0 1px 0 rgba(0,0,0,0.12);    background: rgba(245, 245, 245, 0.2);" class="list-group-item margin-15" href="javascript:void(0)" id="' + k + '"  onMouseOver="toolOptionShow(' + k + ')" onMouseOut="toolOptionHide(' + k + ')">' +
          ...
          '<img src="https://lh3.googleusercontent.com/-R2RhurLgYNY/Vy2hoYXFqqI/AAAAAAAAB1U/mWrX0vaZRToOHzgxYJqTQRQ9TkwBz-mCACL0B/w15-h32-no/rsz_drag_icon.png" width="20px"  onerror = "$(this).remove();"  alt="...">' +
          '</div>' +
          '</div>' +
    ...
    temp += '<button type="button" class="btn btn-sm btn-outline btn-default width-250 font-weight-400 center-block addnewBtn margin-top-20 margin-bottom-50"><i class="icon wb-plus"></i>&nbsp;Add Project</button>';
    ...
    $(".allowSort").sortable({
      ...
      update: function(event, ui) {
        ...
        $.each(sectionOrder, function(k, v) {
          if (v != "") {
            Data.Project[v].order_prefs = orderpref;
            orderpref++;
          }
        });
    ...
    $('div.media-body').on('click', function(e) {
      ...
      if ($('.editform').is(':visible')) {
        ...
        $(".eimg").attr('src',"/static/classic/topbar/assets/images/chars/"+Data.Project[$(this).data('1')].section_title2.charAt(0).toUpperCase()+".png");
        $(".esec2").val(Data.Project[$(this).data('1')].section_title1);
        $(".esec1").val(Data.Project[$(this).data('1')].section_title2);
        $(".esec3").val(Data.Project[$(this).data('1')].section_title3);
        var res = Data.Project[$(this).data('1')].start_date.split(' ');


        $(".estmn").val($.trim(res[0]));
        $(".estyr").val($.trim(res[1]));
        res = Data.Project[$(this).data('1')].end_date.split(' ');
        ...
        $(".edeon").val(Data.Project[$(this).data('1')].more_info1);
        $(".edeon").summernote();
        ...
        $('.eprnt').on("change", function() {
          if ($(".eprnt").prop('checked') == true) {
            ...
            $(".eenmn").prop('disabled', true);
            $(".eenyr").prop('disabled', true);
            $('.PRO-' + $('.eid').html() + '.E2').html($('.esec2').val()+ " &nbsp;|&nbsp; "+ $('.esec3').val() +" &nbsp;|&nbsp; "+$('.estmn').val()+" "+$('.estyr').val()+" - "+ "Present");
          }
          else {
            $(".eenmn").prop('disabled', false);
            $(".eenyr").prop('disabled', false);
            $('.PRO-' + $('.eid').html() + '.E2').html($('.esec2').val()+ " &nbsp;|&nbsp; "+ $('.esec3').val() +" &nbsp;|&nbsp; "+$('.estmn').val()+" "+$('.estyr').val());
          }
        });
        ...
        $('.esec1').on("input", function() {
          $('.PRO-' + $('.eid').html() + '.E1').html($('.esec1').val().toUpperCase());
        });
        ...
        $('.esec2, .esec3, .estyr, .estmn, .eenmn, .eenyr, .eprnt').on("input", function() {
          ...
          $('.PRO-' + $('.eid').html() + '.E2').html(subtitle);
        });
        
        $('.editform .note-editor').on("input", function() {
          $('.PRO-' + $('.eid').html() + '.E3').html($(".edeon").code());
        });
  ...
  function updateRecord(key) {
    ...
    if (Data.Project[key] != null && Data.Project[key].section_type == "PROJECT") {
      Data.Project[key].section_title1 = $(".esec2").val();
      Data.Project[key].section_title2 = $(".esec1").val();
      Data.Project[key].section_title3 = $(".esec3").val();


      Data.Project[key].start_date = $.trim($('.estmn').val())+' '+$.trim($('.estyr').val());


      if($(".eprnt").prop('checked') == true){
        Data.Project[key].end_date = 'Present';
      }else{
        Data.Project[key].end_date = $.trim($('.eenmn').val())+' '+$.trim($('.eenyr').val());
        if($.trim(Data.Project[key].end_date) == '-' || $.trim(Data.Project[key].end_date) == ''){
          Data.Project[key].end_date = '';
        }
      }

      if($(".edeon").code().replace(/<\/?[^>]+(>|$)/g, "") == ""){
        Data.Project[key].more_info1 = "";
      }else{
        Data.Project[key].more_info1 = $(".edeon").code();
      }
      localStorage.setItem("data", JSON.stringify(Data));
      localStorage.setItem("modified_dt", Date.now());
      buildresume();
  ...
  function addRecord() {
    if($(".sec1").val()!=""){
      ...
      param.start_date = ($(".stmn").val().trim() != null)?$(".stmn").val().trim():' '+ ($(".styr").val().trim() != null)?$(".styr").val().trim():'';
      if ($('.prnt').is(":checked")) {
        param.end_date = 'Present';
      }
      else {
        param.end_date = ($(".enmn").val().trim() != null)?$(".enmn").val().trim():' '+ ($(".enyr").val().trim() != null)?$(".enyr").val().trim():'';
      }

      param.more_info1 = $(".deon").code();

      Data.Project.push(param);
      localStorage.setItem("data", JSON.stringify(Data));
      localStorage.setItem("modified_dt", Date.now());
      buildresume();
      $(".deon").code('');
  ...
  function changeVisibility(key, status) {
    ...
    if (status == 1) {
      Data.Project[key].is_active = 0;
    }
    else {
      Data.Project[key].is_active = 1;
    }
    localStorage.setItem("data", JSON.stringify(Data));
    localStorage.setItem("modified_dt", Date.now());
    buildresume();
...
<div class="nav_tab_container">
  ...
  <div style="float:left;width:64%;line-height: 45px;">
    ...
    <ul class="site-sidebar-nav nav nav-tabs nav-justified nav-tabs-line scrollable scrollable-horizontal" role="tablist" >
      ...
      <li style="cursor:pointer;font-weight:400;" role="presentation">
        <a class="active_menu" data-toggle="slidePanel" aria-hidden="true" data-url="../../static/classic/topbar/tpl/PROJECTS.tpl"><span>PROJECTS</span>
        </a>
      </li>
      <li style="cursor:pointer;font-weight:400;" role="presentation">
...
<div class="site-sidebar-tab-content tab-content">
  ...
  <div class="tab-pane fade active in" id="sidebar-editing" style="opacity:0;">
    ...
    <div class="">
      ...
      <div class="">
        <span class="slidePanel-desc">Showcase your projects you have done</span>
        <br>
        <br>
        ...
        <form class='newForm margin-bottom-50 padding-10' style="margin-right: -10px; margin-left: -10px;">
          <div class="newForm-header" style="margin: -10px;margin-bottom:15px;">
            <span class="new-heading pull-right" style=" margin-top: -10px; line-height: 25px; text-align: right;">
              <i class="fa fa-close" style="font-color:red;cursor:pointer;"></i><br>ADD PROJECT
            </span>
            <div class="avatar avatar-sm" style="width:35px;">
              <img src="/static/classic/topbar/assets/images/chars/new.png" alt="...">
            </div>
          </div>
          <!--<span class="new-heading">ADD PROJECT</span><i class="icon wb-close pull-right cancelnewadd" style="cursor:pointer;color:#DE5F5F;margin-right: 8px;"></i>-->
          ...
          <div class="form-group form-material floating">
            ...
            <label class="floating-label">Company/Institute Name</label>
          </div>
        ...
        <div class="row margin-bottom-50">
          ...
          <div class="col-sm-5 col-xs-5 ">
            <button type="button" class="btn btn-block btn-warning font-weight-400" style="letter-spacing:.1em;" data-toggle="slidePanel" aria-hidden="true" data-url="../../static/classic/topbar/tpl/ACHIEVEMENTS_new.tpl">NEXT&nbsp; <i class="fa fa-chevron-right"></i> </button>
          </div>
        </div>
